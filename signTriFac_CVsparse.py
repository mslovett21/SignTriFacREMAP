...
def signLowF(D_pos,D_neg,F_j,P_pos,P_neg, bal,F_i,weight):
    ...
    print("calculating curr_D_pos")
    curr_D_pos    = F_i.dot(P_postFj)
    D_pos_est     = sparse.csr_matrix(curr_D_pos)
    print("currD_neg")
    curr_D_neg    = F_i.dot(P_negtFj)
    D_neg_est     = sparse.csr_matrix(curr_D_neg)
    print("finished D_neg")
    FjtP_pos      = F_j.dot(P_pos.transpose())
    FjtP_neg      = F_j.dot(P_neg.transpose())
    print("Finished transposes")
